...
const AdminPanel: React.FC<AdminPanelProps> = ({ allUsers, allClients, onUpdateUser }) => {
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                ...
                <th className="px-8 py-5">Uso da Cota (Limite 10)</th>
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col gap-2 w-32">
                        <div className="flex justify-between text-[10px] font-bold">
                          ...
                          <span className={userClientsCount >= 10 && !isPro ? 'text-red-500' : 'text-blue-500'}>
                            {Math.round((userClientsCount / (isPro ? userClientsCount || 1 : 10)) * 100)}%
                          </span>
                        </div>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        {!isPro ? (
                          <button 
                            ...
                            className="bg-green-500 hover:bg-green-600 text-white text-[10px] font-black px-4 py-2.5 rounded-xl shadow-lg shadow-green-100 transition-all flex items-center gap-2"
                          >
                          ...
                          <button 
                            ...
                            className="bg-white border border-slate-200 text-slate-400 hover:text-red-500 hover:border-red-100 text-[10px] font-black px-4 py-2.5 rounded-xl transition-all"
                          >
                            REBAIXAR
                          </button>
                        )}
                        ...
                        <button 
                          ...
                          className={`p-2.5 rounded-xl border transition-all ${
                            user.status === 'ativo' 
                              ? 'border-slate-100 text-slate-300 hover:text-red-500 hover:bg-red-50' 
      ...
      <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100 flex items-center gap-6">
        ...
        <div>
          <h4 className="font-bold text-blue-900">Dica do Arquiteto</h4>
          <p className="text-sm text-blue-700/80 leading-relaxed">
            Sempre confira o comprovante no seu WhatsApp (<strong>5581989411840</strong>) antes de clicar em "ATIVAR PRO". 
            Uma vez ativado, o limite de 10 clientes do usuário será removido instantaneamente.
          </p>
        </div>
      </div>
    </div>

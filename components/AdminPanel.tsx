...
const AdminPanel: React.FC<AdminPanelProps> = ({ allUsers, allClients, onUpdateUser, onDeleteUser, currentUser }) => {
  ...
  const stats = {
    ...
    activeToday: Math.floor(allUsers.length * 0.4) // Simulação de usuários ativos
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex gap-2 bg-slate-100 p-1.5 rounded-2xl">
          <button 
            ...
            className={`px-4 py-2 rounded-xl text-xs font-black transition-all ${activeTab === 'users' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500'}`}
          >USUÁRIOS</button>
          <button 
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Conversão Pro', value: `${((stats.proUsers / stats.totalUsers) * 100).toFixed(1)}%`, color: 'bg-amber-500', icon: 'fa-star' },
          ...
          <div className="bg-white rounded-[40px] border border-slate-100 shadow-xl overflow-hidden">
            <div className="p-8 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-3">
                 <button className="px-5 py-3 bg-slate-900 text-white rounded-2xl text-xs font-black hover:bg-black transition-all">
                   EXPORTAR LISTA (CSV)
                 </button>
              </div>
            </div>
      ...
      {activeTab === 'revenue' && (
        <div className="bg-white p-12 rounded-[40px] text-center border border-slate-100 shadow-xl">
          ...
          <div className="max-w-md mx-auto p-6 bg-slate-50 rounded-3xl border border-slate-100">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4">Meta Mensal (R$ 5.000,00)</p>
            <div className="w-full bg-slate-200 h-4 rounded-full overflow-hidden">
               <div className="bg-green-500 h-full transition-all" style={{ width: `${(stats.mrr / 5000) * 100}%` }}></div>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-slate-900 p-8 rounded-[40px] text-white">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-white/10">
                <span className="text-xs font-bold text-slate-400">TEMPO DE RESPOSTA</span>
                <span className="text-xs font-black text-white">42ms</span>
              </div>
              <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                <span className="text-[10px] font-black text-slate-400 uppercase block mb-2">ENDPOINT DE PRODUÇÃO</span>
                <code className="text-[10px] text-blue-300 break-all">postgresql://neon-db-clientesimples-main...</code>
              </div>
            </div>
          </div>
          
          <div className="bg-blue-600 p-8 rounded-[40px] text-white">
             <h3 className="text-xl font-black mb-6">Configuração SaaS</h3>
             <p className="text-sm text-blue-100 mb-8 leading-relaxed">
               As permissões globais do sistema estão configuradas para o modo **Auto-Sync**. 
               Novos cadastros são auditados e sincronizados com a camada de dados a cada ciclo de 10 segundos.
             </p>
             <button className="w-full py-4 bg-white text-blue-600 rounded-2xl font-black hover:bg-blue-50 transition-all">
               REINICIAR SERVIÇOS EDGE
             </button>
          </div>
        </div>
      )}

...
interface LoginProps {
  onLogin: (email: string, pass: string, isRegistering: boolean, name?: string) => void;
  adminEmail: string;
}
...
const Login: React.FC<LoginProps> = ({ onLogin, adminEmail }) => {
  ...
  const isAdminAttempt = email.toLowerCase().trim() === adminEmail.toLowerCase().trim();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      // Passa isRegister para o App.tsx decidir se deve criar conta ou apenas buscar
      onLogin(email, password, isRegister, isRegister ? name : undefined);
    } catch (err: any) {
